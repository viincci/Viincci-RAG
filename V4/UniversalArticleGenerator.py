"""
UniversalArticleGenerator.py - Research V4
Domain-agnostic article generator for any research topic
"""
import requests
import json
import re
...
class HTMLContentFormatter:
    ...
    def __init__(self, image_settings: Dict, content_cleaner: ContentCleaner):
        ...
        self.default_image = image_settings.get("default_fallback", "/img/posts/default.jpg")
...
class WikiCommonsImageFetcher:
    ...
    def get_images_for_topic(self, topic: str, count: int = 5) -> List[Dict[str, Any]]:
        """Get images for any topic"""
        images = self.search_images(topic, limit=count)

        while len(images) < count and len(images) > 0:
            images.append(images[0])

        return images[:count]


def create_image_html(image: Dict[str, Any], topic: str, section_name: str,
...
class UniversalArticleGenerator:
    """Generate structured articles for any research domain"""
    ...
    def __init__(self, config: ConfigManager = None, rag_system=None, fetch_images: bool = True):
        ...
        logger.info(f"Universal Article Generator initialized for domain: {self.domain}")
    ...
    def generate_section(self, section_name: str, topic: str, 
        ...
        """Generic section generator for any domain"""
        section_html = []

        if image:
            section_html.append(create_image_html(
                image, topic, section_name,
        ...
        if self.rag_system and research_data and query:
            ...
            content = default_content or f"Information about {topic} for {section_name}."
    ...
    def get_domain_sections(self, topic: str) -> List[Dict[str, str]]:
        """Get section structure based on domain"""
        questions = self.config.get_domain_questions()
        
        # Generic section templates by domain
        domain_sections = {
            "botany": [
                {"name": "Introduction", "query": f"Write an engaging introduction about {topic}, including its origin and significance"},
                {"name": "Fascinating Facts", "query": f"What are the most interesting facts about {topic}?"},
                {"name": "Care & Cultivation", "query": f"How do you care for and cultivate {topic}?"},
                {"name": "Benefits & Uses", "query": f"What are the benefits and traditional uses of {topic}?"},
                {"name": "Conclusion", "query": f"Summarize the key points about {topic}"}
            ],
            "medical": [
                {"name": "Introduction", "query": f"Write an introduction about {topic} in medical context"},
                {"name": "Symptoms & Signs", "query": f"What are the symptoms and signs of {topic}?"},
                {"name": "Treatments", "query": f"What are the current treatments for {topic}?"},
                {"name": "Prevention", "query": f"How can {topic} be prevented or managed?"},
                {"name": "Conclusion", "query": f"Summarize key medical points about {topic}"}
            ],
            "mathematics": [
                {"name": "Introduction", "query": f"Introduce {topic} in mathematical context"},
                {"name": "Theory & Principles", "query": f"Explain the theoretical foundations of {topic}"},
                {"name": "Applications", "query": f"What are the practical applications of {topic}?"},
                {"name": "Examples", "query": f"Provide clear examples demonstrating {topic}"},
                {"name": "Conclusion", "query": f"Summarize the significance of {topic}"}
            ],
            "carpentry": [
                {"name": "Introduction", "query": f"Introduce {topic} in woodworking context"},
                {"name": "Techniques", "query": f"What are the key techniques for {topic}?"},
                {"name": "Tools & Materials", "query": f"What tools and materials are needed for {topic}?"},
                {"name": "Best Practices", "query": f"What are the best practices and safety tips for {topic}?"},
                {"name": "Conclusion", "query": f"Summarize the essential points about {topic}"}
            ]
        }
        
        # Default generic structure
        default_sections = [
            {"name": "Introduction", "query": f"Introduce {topic}"},
            {"name": "Key Concepts", "query": f"What are the key concepts of {topic}?"},
            {"name": "Practical Information", "query": f"What practical information is important about {topic}?"},
            {"name": "Applications", "query": f"What are the applications or uses of {topic}?"},
            {"name": "Conclusion", "query": f"Summarize {topic}"}
        ]
        
        return domain_sections.get(self.domain, default_sections)

    def generate_full_article(self, topic: str, research_data: List[Dict]) -> str:
        """Generate complete article with all sections for any domain"""
        images = []
        
        if self.fetch_images:
            logger.info(f"Fetching images for {topic}...")
            print(f"Fetching images for {topic}...")
            images = self.image_fetcher.get_images_for_topic(topic)
            logger.info(f"Found {len(images)} images")
            print(f"Found {len(images)} images")

        # Get section structure for domain
        sections_config = self.get_domain_sections(topic)
        
        while len(images) < len(sections_config):
            images.append(None)
        ...
        heading_text = {
            "title": heading["title"].format(plant_name=topic),  # Works for any topic
            "subtitle": heading["subtitle"].format(plant_name=topic)
        }
...
categories: [{self.domain_info.get('name', 'Research')}, {self.domain.title()}]
tags: [{topic.lower()}, {self.domain}, research-guide]
---
...
if __name__ == "__main__":
    ...
    output_file = f'_posts/{datetime.now().strftime("%Y-%m-%d")}-pythagorean-theorem.html'
